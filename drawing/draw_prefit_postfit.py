...
color_procs = {
    ...
    "zee":    "#ffed6f",
    "gsee":   "#fdbf6f",
    "singlemu_pass": "#80b1d3",
    "doublemu_pass": "#fb8072",
    "triplemu_pass": "#b3de69",
}
...
proc_names = {
    ...
    "singlemu_pass": "Single Muon",
    "doublemu_pass": "Double Muon",
    "triplemu_pass": "Triple Muon",
}

conv_region = {
    "monojet":    r'Signal Region',
    "monojetqcd": r'QCD Sideband',
    "singleele":  r'Single Electron',
    "singlemu":   r'Single Muon',
    "singletau":  r'Single Tau',
    "doubleele":  r'Double Electron',
    "doublemu":   r'Double Muon',
}


#bin_real_vals = np.array([200,210,220,230,250,275,300,350,400,500,600,800])
#bin_real_vals = np.array([200,220,250,280,310,340,370,400,430,470,510,550,590,640,690,740,790,840,900,960,1020])
...
def main():
    ...
    with uproot.open(path) as f:
        for region in list(set([region_dir.replace("_prefit", "_postfit") for region_dir in f.keys()])):
            post_fit_region = region
            pre_fit_region = region.replace("_postfit", "_prefit")
            ...
            with uproot.open("/vols/build/cms/sdb15/ZinvWidth/HiggsCombine/ZinvCombine/data/triggers/20190205_metTriggerNew/Zinv_METnoX-ShapeTemplates_met_trigger.root") as f2:
                total_bkg_prefit = f2[post_fit_region.replace("_postfit", "")][post_fit_region.replace("_postfit", "_pass")].pandas()
                total_bkg_prefit.index = total_bkg_prefit.index.rename("bins")
